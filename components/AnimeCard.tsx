import Image from 'next/image';

import Manga from '../public/manga.jpg';

import { MotionDiv } from './Motion';

const stagger = 0.25;

const variants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1 },
};

export interface AnimeProp {
  id: string;
  name: string;
  image: {
    original: string;
    preview: string;
  };
  url: string;
  kind: string;
  chapters: number;
  episodes: number;
  episodes_aired: number;
  score: string;
}

interface Prop {
  anime: AnimeProp;
  index: number;
}

function AnimeCard({ anime, index }: Prop) {
  console.log(' anime: ', anime);
  return (
    <MotionDiv
      variants={variants}
      initial='hidden'
      animate='visible'
      transition={{
        delay: index * stagger,
        ease: 'easeInOut',
        duration: 0.5,
      }}
      viewport={{ amount: 0 }}
      className='max-w-sm rounded relative w-full hadow-red-900 shadow-md'
    >
      <div className='relative w-full h-[32vh]'>
        {anime.image && (
          <Image
            src={anime.image.original ? `https://shikimori.one${anime.image.original}` : Manga}
            alt={anime.name || 'manga poster'}
            fill
            sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw'
            quality={100}
            className='rounded-xl object-cover'
          />
        )}
      </div>
      <div className='py-4 flex flex-col gap-3'>
        <div className='flex justify-between items-center gap-1'>
          <h2 className='font-bold text-white text-xl line-clamp-1 w-full'>{anime.name}</h2>
          <div className='py-1 px-2 bg-[#161921] rounded-sm'>
            <p className='text-white text-sm font-bold capitalize'>{anime.kind}</p>
          </div>
        </div>
        <div className='flex gap-4 items-center'>
          <div className='flex flex-row gap-2 items-center'>
            <Image
              src='./episodes.svg'
              alt='episodes'
              width={20}
              height={20}
              className='object-contain'
            />
            <p className='text-base text-white font-bold'>{anime.chapters}</p>
          </div>
          <div className='flex flex-row gap-2 items-center'>
            <Image src='./star.svg' alt='star' width={18} height={18} className='object-contain' />
            <p className='text-base font-bold text-[#ffdc2c]'>{anime.score}</p>
          </div>
        </div>
      </div>
    </MotionDiv>
  );
}

export default AnimeCard;
